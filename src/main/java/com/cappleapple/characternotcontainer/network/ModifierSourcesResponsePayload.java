...
public record ModifierSourcesResponsePayload(
    ...
    public static final net.minecraft.resources.ResourceLocation TYPE = CharacterNotContainer.id("modifier_sources_response");
    public static final PacketCodec<ModifierSourcesResponsePayload> STREAM_CODEC = new PacketCodec<>() {
        @Override
        public ModifierSourcesResponsePayload decode(FriendlyByteBuf buffer) {
            ...
            for (int index = 0; index < size; index++) {
                ...
                java.util.UUID modifier = buffer.readUUID();
                ...
                for (int sourceIndex = 0; sourceIndex < sourceCount; sourceIndex++) {
                    ItemStack stack = buffer.readBoolean() ? buffer.readItem() : ItemStack.EMPTY;
        ...
        @Override
        public void encode(FriendlyByteBuf buffer, ModifierSourcesResponsePayload payload) {
            ...
            for (int index = 0; index < size; index++) {
                ...
                buffer.writeUUID(entry.modifierId);
                ...
                for (int sourceIndex = 0; sourceIndex < sourceCount; sourceIndex++) {
                    ...
                    if (!source.stack.isEmpty()) buffer.writeItem(source.stack);
    ...
    public record Entry(ResourceLocation attributeId, java.util.UUID modifierId, List<Source> sources) {
